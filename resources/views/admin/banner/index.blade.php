...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '/admin/banner/getBannerList?_token={{ csrf_token() }}',
            method: 'post',
            toolbar: '#toolbarDemo',
            defaultToolbar: ['filter'],
            cols: [[
                {
                    field: 'img', width: 150, title: '缩略图', align: 'center', templet: function (d) {
                        return '<img class="layui-upload-img" width="200px" src="' + d.img + '">';
                    }
                },
                {field: 'remark', minWidth: 150, title: '备注'},
                {field: 'url', minWidth: 200, title: '跳转链接'},
                {field: 'sort', width: 80, title: '排序', align: "center"},
                {
                    field: 'status', width: 80, align: "center", templet: function (d) {
                        if (d.status === 1) {
                            return '<i class="layui-icon layui-icon-ok" style="color: #393;" lay-event="setBannerStatus"></i>';
                        } else if (d.status === 2) {
                            return '<i class="layui-icon layui-icon-close" style="color: #f00;" lay-event="setBannerStatus"></i>'
                        }
                    }, title: '状态'
                },
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '/admin/banner/edit?banner_id=' + data.banner_id,
                });
                $(window).on("resize", function () {
                    layer.full(index);
                });
            ...
            } else if (obj.event === 'setBannerStatus') {

                var status = data.status === 1 ? 2 : 1;
                $.ajax({
                    url: "/admin/banner/setBannerStatus?_token={{ csrf_token() }}",
                    type: "post",
                    dataType: "json",
                    data: {
