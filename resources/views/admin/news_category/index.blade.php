            ...
            cols: [[
                ...
                {field: 'sort', width: 80, title: '排序'},
                {
                    field: 'status', minWidth: 80, align: "center", templet: function (d) {
                        if (d.status === 1) {
                            return '<i class="layui-icon layui-icon-ok" style="color: #393;" lay-event="setNewsCategoryStatus"></i>';
                        } else if (d.status === 2) {
                            return '<i class="layui-icon layui-icon-close" style="color: #f00;" lay-event="setNewsCategoryStatus"></i>'
                        }
                    }, title: '状态'
                },
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {   // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '/admin/newsCategory/edit',
                });
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '/admin/newsCategory/edit?category_id=' + data.category_id,
                });
                $(window).on("resize", function () {
                    layer.full(index);
                });
            ...
            } else if (obj.event === 'setNewsCategoryStatus') {

                var status = data.status === 1 ? 2 : 1;
                $.ajax({
                    url: "/admin/newsCategory/setNewsCategoryStatus?_token={{ csrf_token() }}",
                    type: "post",
                    dataType: "json",
                    data: {
