    ...
    <div class="layuimini-main">
        ...
        <div class="layui-form layuimini-form">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="sort" value="{{!empty($categoryInfo['sort']) ? $categoryInfo['sort'] : 0}}" class="layui-input" placeholder="请输入排序（值越大，该产品在前台列表排序越靠前，值不可大于127）" lay-verify="number" oninput="if(value>127)value=127;if(value<0)value=0">
                    <span class="layui-badge-dot layui-bg-orange"></span><span class="layui-badge layui-bg-gray">排序值越大，该产品在前台列表排序越靠前，值不可大于127</span>
                </div>
            </div>

            <div class="layui-form-item">
                <label class="layui-form-label required">栏目图片</label>
                <div class="layui-input-block">
                    <div class="layui-upload">
                        <button type="button" class="layui-btn" id="test1">上传图片</button>
                        <div class="layui-upload-list">
                            <img class="layui-upload-img" id="demo1" width="200px" src="{{!empty($categoryInfo['img']) ? $categoryInfo['img'] : ''}}">
                            <input type="hidden" name="img" id="img" value="{{!empty($categoryInfo['img']) ? $categoryInfo['img'] : ''}}">
                            <p id="demoText"></p>
                        </div>
...
<script>
    layui.use(['jquery', 'form', 'upload', 'element'], function () {
        ...
        form.on('submit(saveBtn)', function (data) {
            ...
            $.ajax({
                url: '/admin/productCategory/doEdit?_token={{ csrf_token() }}',
                type: 'POST',
                dataType: 'json',
                data: data.field,
        ...
        var uploadInst = upload.render({
            elem: '#test1'
            ,url: '/admin/api/uploadCategoryImg?_token={{ csrf_token() }}' //改成您自己的上传接口
            ,before: function(obj){
                //预读本地文件示例，不支持ie8
